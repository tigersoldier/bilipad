...
export class DynamicList extends ContainerControl {
    ...
    for (const mutation of mutations) {
      for (const node of mutation.addedNodes) {
        if (
          ...
          node.classList.contains(LIST_ITEM_CLASS)
        ) {
          this.addItem(node);
        }
      }
  ...
  override onGamepadButtonEvent(event: GamepadButtonEvent): boolean {
    if (
      ...
      event.buttonId == ButtonId.DPAD_DOWN
    ) {
      this.items[0].focus();
      return true;
    }
...
export class DynamicListItem extends ContainerChildControl(
  BaseContainerChildControl,
) {
  ...
  constructor(element: HTMLElement, parent: DynamicList, index: number) {
    ...
    const contentElement = this.itemElement.querySelector(
      ".bili-dyn-item__main",
    );
    if (contentElement) {
      ...
      const observer = new MutationObserver((mutations) => {
        for (const mutation of mutations) {
          for (const node of mutation.addedNodes) {
            if (
              node instanceof HTMLElement &&
              node.classList.contains("bili-dyn-item__main")
            ) {
              this.$hasContent = true;
              this.updateContentElements();
              observer.disconnect();
  ...
  private updateContentElements() {
    ...
    this.forwardButtonElement = this.itemElement.querySelector(
      ".bili-dyn-item__action .bili-dyn-action.forward",
    ) as HTMLElement | undefined;
    this.commentButtonElement = this.itemElement.querySelector(
      ".bili-dyn-item__action .bili-dyn-action.comment",
    ) as HTMLElement | undefined;
    this.likeButtonElement = this.itemElement.querySelector(
      ".bili-dyn-item__action .bili-dyn-action.like",
    ) as HTMLElement | undefined;
    this.titleElement = this.itemElement.querySelector(".bili-dyn-title") as
      | HTMLElement
      | undefined;
    this.descriptionElement = this.itemElement.querySelector(
      ".bili-dyn-time",
    ) as HTMLElement | undefined;
    this.videoCardElement = this.itemElement.querySelector(
      ".bili-dyn-card-video",
    ) as HTMLElement | undefined;
  }
